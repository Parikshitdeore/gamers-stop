  ...
  return (
    <nav className="nav-main">
      <div
        ...
        style={{
          boxShadow: home
            ? "1px 1px 10px 0.5px rgba(255,255,255)"
            : "1px 1px 3px 2px rgba(0,0,0,0.2)",
        }}
      >
        <div className="logo-container" onClick={() => navigate("/")}>
          <img src={logo} alt="none" />
          <h3 style={{ color: home ? "white" : "#6e6db4", background: "none" }}>
            Gamer's Stop
          </h3>
        </div>
        <div className="search-container">
          <input
            ...
            onChange={(event) => {
              setFilsearch(event.target.value.toLowerCase().replace(" ", ""));
            }}
        ...
        <div className="links-container">
          <NavLink to={home ? "/products" : "/"} className="nav-links">
            <div
              style={{ display: home ? "" : "none" }}
              className="icon-and-link"
            >
              <Icon icon="mdi:search" color="#915dc2" />
              <p style={{ color: home ? "white" : "black" }}>
                {home ? "Products" : "Home"}
              </p>
            </div>
          </NavLink>
